...
public class ValidationService
{
    ...
    public ValidationReport ValidateEnvironment(string environmentId)
    {
        ...
        var duplicateGroups = routes
            ...
            .GroupBy(
                x => new PathMethodKey { UpstreamPathTemplate = x.route.UpstreamPathTemplate ?? string.Empty, Method = x.method },
                PathMethodKeyComparer.Instance)
            .Where(g => g.Count() > 1);
    ...
    private sealed record PathMethodKey
    {
    ...
    private sealed class PathMethodKeyComparer : IEqualityComparer<PathMethodKey>
    {
        ...
        public bool Equals(PathMethodKey? x, PathMethodKey? y)
        {
            ...
            return string.Equals(x.UpstreamPathTemplate, y.UpstreamPathTemplate, StringComparison.OrdinalIgnoreCase)
        ...
        public int GetHashCode(PathMethodKey obj)
        {
            return HashCode.Combine(
                StringComparer.OrdinalIgnoreCase.GetHashCode(obj.UpstreamPathTemplate ?? string.Empty),
                StringComparer.OrdinalIgnoreCase.GetHashCode(obj.Method ?? string.Empty));
        }
    }
}
